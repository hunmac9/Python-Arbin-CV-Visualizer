...
def get_color_palettes(config_file, section='PALETTES'):
    """
    Manually parse the configuration file and get the keys from
    ...
    with open(config_file, 'r') as file:
        for line in file:
            ...
            # Start capturing if the section starts
            if line == f'[{section}]':
            ...
            # Capture keys if in the section
...
def browse_file():
    ...
    if filepath:
        ...
        # Load the Excel data to extract the mass
        try:
...
def update_palette_preview(event=None):
    ...
    if color_palette.lower() == 'custom':
        ...
        start_color_entry.grid(row=14, column=1, padx=10, pady=5)
        start_color_button.grid(row=14, column=2, padx=10, pady=5)
        end_color_entry.grid(row=15, column=1, padx=10, pady=5)
        end_color_button.grid(row=15, column=2, padx=10, pady=5)
    else:
        colors = config['PALETTES'][color_palette].split(',')
...
# Mass display
tk.Label(root, text="Mass (g):").grid(row=1, column=0, padx=10, pady=5, sticky=tk.W)
mass_var = tk.StringVar()
tk.Entry(root, textvariable=mass_var, state='readonly').grid(row=1, column=1, padx=10, pady=5)
